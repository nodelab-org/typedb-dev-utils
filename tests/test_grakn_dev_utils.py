...
def test_ls_types(database_params, db_client):
    gradevils.ls_types(database_params["database"], client=db_client)
    assert True

def test_def_attr_type_and_get_type_owns(database_params, db_client):
    gradevils.def_attr_type(database=database_params["database"], new_attr_label = "test_attr", new_attr_value="string", sup_label="attribute", is_key=False, thingTypes=["person"], verbose=True, client=db_client)
    assert all([attr in gradevils.get_type_owns(database_params["database"], thingType = "person", client=db_client) for attr in ["test_attr"]])


def test_def_rel_type(database_params, db_client):
    client = gradevils.def_rel_type(database_params["database"], "association", dict_role_players={"associate":{"role_players":["house","person"], "role_sup":"role"}}, rel_sup="relation", verbose=True, client=db_client, return_client = True)
...
def test_insert_data(database_params, db_client):
    client = gradevils.def_attr_type(database_params["database"], new_attr_label = "UID", new_attr_value="string", sup_label="attribute", is_key=False, rootTypes=["entity","relation","attribute"], client=db_client, return_client = True)
...
def test_ls_instances(database_params, db_client):
    client = gradevils.insert_data(database_params["database"], database_params["gql_data"], parse_lines=True, client=db_client, return_client = True)
