...
def test_def_attr_type_and_get_type_owns(database_params, db_client):
    db_client.close()
    gradevils.def_attr_type(database=database_params["database"], new_attr_label = "test_attr", new_attr_value="string", sup_label="attribute", is_key=False, thingTypes=["person"], verbose=True)
...
def test_insert_data(database_params, db_client):
    db_client.close()
    gradevils.def_attr_type(database_params["database"], new_attr_label = "UID", new_attr_value="string", sup_label="attribute", is_key=False, rootTypes=["entity","relation","attribute"])
